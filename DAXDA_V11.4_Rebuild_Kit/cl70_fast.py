"""Optimized Clifford Algebra Cl(7,0) — 7-Dimensional Conformal Space.

128-blade algebra with NumPy array backend for maximum throughput.
NO lookup tables, NO scaffolding — pure algebraic computation.

Basis: 7 vectors (e1..e7), signature (+,+,+,+,+,+,+).
  e1^2 = e2^2 = e3^2 = e4^2 = e5^2 = e6^2 = e7^2 = +1

Blade count: 2^7 = 128.
Multiplication table: 128x128 = 16,384 entries (fits in L1 cache).
"""
...
P, Q = 7, 0  # Cl(7,0) positive-definite algebra
N = P + Q    # 7
DIM = 1 << N # 128

# Metric: all 7 basis vectors square to +1
METRIC = np.array([1] * P, dtype=np.int8)
...
def _canonical_product(a: int, b: int) -> tuple[int, int]:
    ...
    Sign accounts for anticommutation swaps and metric contractions.
    """
    ...
    # Cancel paired indices using metric (+1)
    result_indices = []
    i = 0
    while i < len(merged):
...
# ──────────────────────────────────────────────────────────────────────
# Precomputed multiplication table (128x128 = 16,384 entries)
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
# Fast Multivector — NumPy array backend for Cl(7,0)
# ──────────────────────────────────────────────────────────────────────

class FMV:
    """Fast Multivector over Cl(7,0) using a flat NumPy array of 128 components.
    
    Vectorized operations with maximum throughput.
    """
    __slots__ = ('data',)
